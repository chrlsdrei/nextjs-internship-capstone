...
import type { BoardList, BoardTask } from "@/lib/db/queries/board"
...
export function TaskCard({
  ...
  const [moveState, moveAction, moving] = useActionState(moveTaskAction, initialBoardActionState)
  const [deleteState, deleteAction, deleting] = useActionState(deleteTaskAction, initialBoardActionState)
  const [reorderState, reorderAction, reordering] = useActionState(reorderTasksAction, initialBoardActionState)
  const priorityClass = {
    ...
    <article className="rounded-lg border border-french-gray-300 bg-white p-4 shadow-sm dark:border-paynes-gray-400 dark:bg-outer-space-300">
      <div className="flex items-start gap-2">
        ...
        >
          ...
          {task.description && (
            <p className="mt-2 line-clamp-3 text-sm text-paynes-gray-500 dark:text-french-gray-400">
              {task.description}
            </p>
          )}
        </button>
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-french-gray-300 pt-3 dark:border-paynes-gray-400">
        ...
        <form action={moveAction} className="ml-auto flex items-center gap-1">
          ...
          <button type="submit" disabled={moving} className="text-xs text-blue-munsell-600 disabled:opacity-50">
            Move
          </button>
        </form>
        {canDelete && (
      ...
      <Feedback error={moveState.error || deleteState.error || reorderState.error} />
    </article>
  )
}
